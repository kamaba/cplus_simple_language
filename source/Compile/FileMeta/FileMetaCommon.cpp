...
void FileMetaCallNode::CreateFileMetaCallNode() {
    ...
    if (m_Node->NodeType() == SimpleLanguage::ENodeType::Par) {
...
std::string FileMetaCallNode::GetName() const {
    if (m_CallNodeList.size() >= 1) {
...
std::string FileMetaCallNode::ToFormatString() const {
    ...
    if (m_FileMetaParTerm != nullptr) {
        ...
        sb << (m_AtToken ? m_AtToken->lexeme().ToString() : "");
        sb << (token() ? token()->ToConstString() : "");
        if (m_IsArray) {
            sb << (m_BeginBracketToken ? m_BeginBracketToken->lexeme().ToString() : "");
            for (size_t i = 0; i < m_ArrayNodeList.size(); i++) {
                sb << m_ArrayNodeList[i]->ToFormatString();
                if (i < m_ArrayNodeList.size() - 1)
                    sb << ",";
            }
            sb << (m_EndBracketToken ? m_EndBracketToken->lexeme().ToString() : "");
        }
        if (m_IsTemplate) {
            sb << (m_BeginAngleToken ? m_BeginAngleToken->lexeme().ToString() : "");
            ...
            sb << (m_EndAngleToken ? m_EndAngleToken->lexeme().ToString() : "");
        }
        if (m_IsCallFunction) {
...
void FileMetaCallLink::AddChildExtendLinkList(Node* cnode, bool isIncludeSelf) {
    ...
    for (size_t i = 0; i < childNodeList.size(); i++) {
        ...
        if (i == childNodeList.size() - 1) {
            if (cnode1->extendLinkNodeList().size() > 0) {
...
std::vector<std::string> FileMetaClassDefine::stringList() const {
    return SimpleLanguage::Core::FileMetatUtil::GetLinkStringMidPeriodList(m_TokenList);
}

std::string FileMetaClassDefine::allName() const {
    std::ostringstream sb;
    for (size_t i = 0; i < m_TokenList.size(); i++) {
        sb << (m_TokenList[i] ? m_TokenList[i]->lexeme().ToString() : "");
    }
    return sb.str();
}
...
std::string FileMetaClassDefine::ToTokenString() const {
    return allName() + " Token File:[" + (m_ClassNameToken ? m_ClassNameToken->path() : "") + "] Line:[" + 
           (m_ClassNameToken ? std::to_string(m_ClassNameToken->sourceBeginLine()) : "") + "]  Char:[" + 
...
std::string FileMetaClassDefine::ToFormatString() const {
    ...
    if (m_IsInputTemplateData) {
        sb << (m_AngleTokenBegin ? m_AngleTokenBegin->lexeme().ToString() : "");
        for (size_t i = 0; i < m_InputTemplateNodeList.size(); i++) {
            sb << m_InputTemplateNodeList[i]->ToFormatString();
            if (i < m_InputTemplateNodeList.size() - 1) {
                sb << ",";
        ...
        sb << (m_AngleTokenEnd ? m_AngleTokenEnd->lexeme().ToString() : "");
    }
    if (m_IsArray) {
        sb << (m_BracketTokenBegin ? m_BracketTokenBegin->lexeme().ToString() : "");
        for (size_t i = 0; i < m_ArrayTokenList.size(); i++) {
            sb << m_ArrayTokenList[i]->lexeme().ToString();
            if (i < m_ArrayTokenList.size() - 1)
                sb << ",";
        }
        sb << (m_BracketTokenEnd ? m_BracketTokenEnd->lexeme().ToString() : "");
    }
    return sb.str();
}
...
void FileMetaClassDefine::AddError2(int errorId, const std::string& pfile, const std::string& pfunction, int line) {
    ...
    str = str + " \n Token: 在文件:" + (m_ClassNameToken ? m_ClassNameToken->path() : "") + " 开始行号:" + 
...
FileMetaTemplateDefine::FileMetaTemplateDefine(FileMeta* fm, Node* node) : m_FileMeta(fm), m_Node(node) {
    m_Token = node->token();
...
FileMetaTemplateDefine::FileMetaTemplateDefine(FileMeta* fm, Node* node, Node* extendsNode) : m_FileMeta(fm), m_Node(node), m_ExtendsNode(extendsNode) {
    m_Token = node->token();
}
